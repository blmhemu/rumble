...
pub fn get_master_playlist(media_file: &str) -> Result<impl warp::Reply, warp::Rejection> {
    ...
    let a = media_info::get_stream_info(media_file, media_info::StreamType::Subtitle);
    if a.is_err() {
        return Err(warp::reject::not_found());
    }
    ...
    //TODO: Improve this text. It works but is a mess at the moment.
    for (pos, s) in a.streams.iter().enumerate() {
        playlist.push_str(r#"#EXT-X-MEDIA:TYPE=SUBTITLES,GROUP-ID="subs","#);
        ...
        let uri = &format!("/subs/{}/{:02}/", media_file, pos);
        playlist.push_str(&format!(r#"AUTOSELECT=NO,DEFAULT=NO,URI="{}""#, uri));
        playlist.push_str("\n");
    }
    playlist.push_str("\n");
    ...
    playlist.push_str(r#"#EXT-X-STREAM-INF:BANDWIDTH=800000,RESOLUTION=640x360,AUDIO="bipbop_audio",SUBTITLES="subs""#);
    playlist.push_str("\n");
    playlist.push_str(&format!("/video/{}/360/\n", media_file));

    playlist.push_str(r#"#EXT-X-STREAM-INF:BANDWIDTH=1400000,RESOLUTION=842x480,AUDIO="bipbop_audio",SUBTITLES="subs""#);
    playlist.push_str("\n");
    playlist.push_str(&format!("/video/{}/480/\n", media_file));

    playlist.push_str(r#"#EXT-X-STREAM-INF:BANDWIDTH=2800000,RESOLUTION=1280x720,AUDIO="bipbop_audio",SUBTITLES="subs""#);
    playlist.push_str("\n");
    playlist.push_str(&format!("/video/{}/720/\n", media_file));

    playlist.push_str(r#"#EXT-X-STREAM-INF:BANDWIDTH=5000000,RESOLUTION=1920x1080,AUDIO="bipbop_audio",SUBTITLES="subs""#);
    playlist.push_str("\n");
    playlist.push_str(&format!("/video/{}/1080/\n", media_file));
...
pub async fn res_playlist_handler(
    media_file: String,
    resolution: usize,
) -> Result<impl warp::Reply, warp::Rejection> {
...
pub async fn audio_playlist_handler(
    media_file: String,
    index: u8,
) -> Result<impl warp::Reply, warp::Rejection> {
...
fn get_audio_playlist(media_file: &str, index: u8) -> Result<impl warp::Reply, warp::Rejection> {
    let mut playlist = String::new();
    playlist.push_str("#EXTM3U\n");
    playlist.push_str("#EXT-X-VERSION:4\n");
    playlist.push_str(&format!(
...
pub async fn subs_playlist_handler(
    media_file: String,
    index: u8,
) -> Result<impl warp::Reply, warp::Rejection> {
    get_subs_playlist(&media_file, index)
}

fn get_subs_playlist(media_file: &str, index: u8) -> Result<impl warp::Reply, warp::Rejection> {
    let leftover = media_info::get_duration(media_file);
    if leftover.is_err() {
        return Err(warp::reject::not_found());
    }
    ...
    let mut segmentIndex = 0;
    playlist.push_str(&format!("#EXTINF:{:.4},\n", leftover));
    playlist.push_str(&format!("/subs/{}/{:02}/0000.webvtt\n", media_file, index,));
